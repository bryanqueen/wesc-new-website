  ...
  return (
    ...
    >
      <div className="container mx-auto flex h-24 items-center justify-between px-6">
        ...
        <nav className="hidden md:flex items-center space-x-12">
          {navItems.map((item) => (
            <Link
              ...
              className="text-sm tracking-wide hover:text-[--accent] transition-colors text-white"
            >
              {item.toUpperCase()}
            </Link>
          ))}
        ...
        <Link
          href="/contact"
          className="hidden md:inline-flex rounded-full bg-[--primary] px-6 py-3 text-sm text-white hover:bg-[--primary]/90 transition-colors"
        >
          APPLY FOR ELIGIBILITY
        </Link>
            <motion.div 
            className='absolute md-hidden bg-[--primary] rounded-xl'
            variants={menu}
